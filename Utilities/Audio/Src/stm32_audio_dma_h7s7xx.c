...
int32_t UTIL_AUDIO_DMA_Init(UTIL_AUDIO_DMA_t const *const pHdle)
{
  ...
  if (error == UTIL_AUDIO_ERROR_NONE)
  {
    ...
    pCachedCtx->hDma.InitLinkedList.LinkAllocatedPort = DMA_LINK_ALLOCATED_PORT0; // Todo try PORT1
  ...
  if (error == UTIL_AUDIO_ERROR_NONE)
  {
    switch (pHdle->type)
    {
      ...
      case UTIL_AUDIO_DMA_IP_TYPE_MDF:
        /* Associate the DMA handle */
        if (pHdle->direction == DMA_PERIPH_TO_MEMORY)
        {
          MDF_HandleTypeDef *hmdf = (MDF_HandleTypeDef *)pHdle->pIpHdle;
          __HAL_LINKDMA(hmdf, hdma, pCachedCtx->hDma);
        }
        else
        {
          error = UTIL_AUDIO_ERROR;

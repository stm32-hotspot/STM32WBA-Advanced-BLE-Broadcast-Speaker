/**
******************************************************************************
* @file    stm32_usart_h7rsxx_template.c
...
/*
  by default, we assume that the module will be used with freertos, so , priorties must be lower configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY(5)
  to make sure the UART/DMA IT won't be preempted by freertos to ensure an high BW and no errors , if used in bare-metal, the level can be freely adapted
*/

#ifndef UTIL_UART_PRIO
  #define UTIL_UART_PRIO                        8
#endif
...
#ifndef UTIL_UART_TIMx_PRIORITY
  #define UTIL_UART_TIMx_PRIORITY               4
#endif


VAR_DECLARE_ALIGN4_AT_SECTION(static DMA_NodeTypeDef, hTxDmaNode, UTIL_UART_MEMORY_SECTION_HW_BUFFERS); /*cstat !MISRAC2012-Rule-1.4_a extended language features needed*/
VAR_DECLARE_ALIGN4_AT_SECTION(static DMA_NodeTypeDef, hRxDmaNode, UTIL_UART_MEMORY_SECTION_HW_BUFFERS); /*cstat !MISRAC2012-Rule-1.4_a extended language features needed*/

static DMA_QListTypeDef hTxtDmaQ;
static DMA_QListTypeDef hRxtDmaQ;
...
static void s_initTimeoutIT(TIM_HandleTypeDef *pHdle, uint32_t itFreqMs)
{
  ...
  /* hot fix to clean  later * to be at 100ms we must x4 */
  //itFreqMs *= 4;

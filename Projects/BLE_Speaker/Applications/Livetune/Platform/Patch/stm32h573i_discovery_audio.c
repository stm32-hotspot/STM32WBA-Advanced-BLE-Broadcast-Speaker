  ...
  else
  {
    ...
    else /* (Instance == 1U) */
    {
      ...
      else
      {
        ...
        mx_config.FrameLength       = 16; //TodoJO 16;
...
int32_t BSP_AUDIO_IN_RecordPDM(uint32_t Instance, uint8_t *pBuf, uint32_t NbrOfBytes)
{
  ...
  {
    /* Start the process receive DMA */
    /* BSP patch here, we shouldn't use BitsPerSample since DMA is always in Bytes unlike for SAI_I2S nor DFSDM where dma is configured as function of BitsPerSample*/
    //    if (HAL_SAI_Receive_DMA(&haudio_in_sai, (uint8_t *)pBuf, (uint16_t)(NbrOfBytes / (Audio_In_Ctx[Instance].BitsPerSample / 8U))) != HAL_OK)
...
__weak HAL_StatusTypeDef MX_SAI1_Init(SAI_HandleTypeDef *hsai, MX_SAI_Config_t *MXConfig)
{
  ...
  if (MXConfig->AudioFrequency == AUDIO_FREQUENCY_8K)
  {
    /* Set PDM clock to 1024KHz so Mckdiv to 24(Fsai/(Fpdm*2)) ; Mind that MP23 is in low power mode with such setting, not standard mode */
    hsai->Init.Mckdiv = 24U;
    // ret = HAL_ERROR; //this is the reason of the patch; we never use 1024 because ST mems won't support it, we use 2048 instead but 8kHz can't be supported with PDM2PCM because 256 ratio doesn't exists
  }
  else if ((MXConfig->AudioFrequency == AUDIO_FREQUENCY_16K) || (MXConfig->AudioFrequency == AUDIO_FREQUENCY_32K))
  {
    /* Set PDM clock to 2048KHz so Mckdiv to 12 (Fsai/(Fpdm*2)) */
    hsai->Init.Mckdiv = 12U; // this is the reason of the patch; we never use 1024 because ST mems won't support it, we use 2048 instead
  }
  ...
  //  hsai->SlotInit.SlotNumber         = 1; //2; TodoJO
  //  hsai->SlotInit.SlotActive         = SAI_SLOTACTIVE_0; //MXConfig->SlotActive; TodoJO
...
#ifndef UTIL_AUDIO_SAI_PDM_USED
#if (USE_HAL_SAI_REGISTER_CALLBACKS == 0) || !defined (USE_HAL_SAI_REGISTER_CALLBACKS)
/**

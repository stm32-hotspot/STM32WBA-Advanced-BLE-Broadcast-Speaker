...
/* Private macros ------------------------------------------------------------*/
/* USER CODE BEGIN PM */
#define VOLUME_VCP_TO_VOLUME_BSP(Volume) ((uint32_t)roundf(47.4f*log10f(((float) Volume)/2.0f + 1.0f)))
/* USER CODE END PM */
...
static uint32_t Current_Volume = 0x80;
/* USER CODE END PV */
                  ...
                  uint8_t *pSrcBuff)
{
  ...
  SpeakerAudioConfig_t driver_mode = CONFIG_SIMPLEX;
  BSP_AUDIO_Init_t audio_conf;

  if (pSrcBuff != NULL)
  {
    driver_mode = CONFIG_DUPLEX;
  }
  ...
  Speaker_audio_start(driver_mode, audioFrequency, (uint8_t)VOLUME_VCP_TO_VOLUME_BSP(Current_Volume));

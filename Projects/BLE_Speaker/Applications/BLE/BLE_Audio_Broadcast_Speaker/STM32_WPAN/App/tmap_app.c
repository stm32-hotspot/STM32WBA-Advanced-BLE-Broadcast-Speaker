...
#define ADV_INTERVAL_MIN                       (0x0200)
...
void TMAPAPP_Init(uint8_t csip_config_id)
{
  ...
  if (csip_config_id == 0)
  {
    status = SetGapAppearance(GAP_APPEARANCE_STANDALONE_SPEAKER);
  }
  else
  {
  ...
  if (VCP_RENDER_SetAbsVolume(Volume) != 0){
    Error_Handler();
  }
#endif
}
                                 ...
                                 uint16_t Appearance)
{
  ...
  uint8_t Adv_Data[ADV_LEN] = {(ADV_AD_FLAGS_LEN-1), AD_TYPE_FLAGS, (FLAG_BIT_LE_GENERAL_DISCOVERABLE_MODE|FLAG_BIT_BR_EDR_NOT_SUPPORTED),
                               (ADV_LOCAL_NAME_LEN-1), AD_TYPE_COMPLETE_LOCAL_NAME,
                                'S', 'T', 'M', '3', '2', 'W', 'B', 'A', '_', 'P', 'O', 'C', '_', '0', '0', '0', '0' };
  ...
  if (status == BLE_STATUS_SUCCESS)
  {
    App_Notify_Evt(START_ADVERTISING);
  }
  return status;
}
...
uint8_t TMAPAPP_StopAdvertising(void)
{
  ...
  if (status == BLE_STATUS_SUCCESS)
  {
    App_Notify_Evt(STOP_ADVERTISING);
  }

...
uint16_t aSrcBuff[SAI_SRC_MAX_BUFF_SIZE/(10u/UTIL_AUDIO_N_MS_PER_INTERRUPT)] __attribute__((aligned));
uint16_t aLC3InBuff[SAI_SRC_MAX_BUFF_SIZE] __attribute__((aligned));
#else
uint16_t *aSrcBuff = NULL;
uint16_t *aLC3InBuff = NULL;
#endif /*(SAI_SRC_MAX_BUFF_SIZE != 0)*/
#if (SAI_SNK_MAX_BUFF_SIZE != 0)
uint16_t aSnkBuff[SAI_SNK_MAX_BUFF_SIZE/(10u/UTIL_AUDIO_N_MS_PER_INTERRUPT)] __attribute__((aligned));
uint16_t aLC3OutBuff[SAI_SNK_MAX_BUFF_SIZE] __attribute__((aligned));
#else
uint16_t *aSnkBuff = NULL;
uint16_t *aLC3OutBuff = NULL;
#endif /*(SAI_SNK_MAX_BUFF_SIZE != 0)*/

int8_t *UtilAudio_InBuff = (int8_t*)aLC3OutBuff;
int8_t *UtilAudio_OutBuff = (int8_t*)aSnkBuff;

int8_t* UtilAudio_MicPDMScratchBuff = NULL; /* not used */

int8_t  *UtilAudio_InBuffAlternate = (int8_t*)aSrcBuff;
int8_t  *UtilAudio_OutBuffAlternate = (int8_t*)aLC3InBuff;
                                 ...
                                 uint16_t Appearance)
{
  ...
  Primary_Adv_Interval_Min = ADV_INTERVAL_MIN;
  Primary_Adv_Interval_Max = ADV_INTERVAL_MIN;
  LOG_INFO_APP("Start Advertising\n");
...
void TMAPAPP_SwitchLanguage(void)
{
  SubgroupID = (SubgroupID + 1) % TMAPAPP_Context.BSNK.base_group.NumSubgroups;
}
...
void APP_NotifyTxAudioCplt(uint16_t AudioFrameSize)
{
  ...
  for (i = 0; i< APP_MAX_NUM_CIS; i++)
  {
    ...
    {
      CODEC_ReceiveData(TMAPAPP_Context.cis_snk_handle[i], 1, &aLC3OutBuff[0] + AudioFrameSize/2 + i);
    }
  }
#if ((APP_TMAP_ROLE & TMAP_ROLE_BROADCAST_MEDIA_RECEIVER) == TMAP_ROLE_BROADCAST_MEDIA_RECEIVER)
  if (TMAPAPP_Context.bap_role & BAP_ROLE_BROADCAST_SINK)
  {
    if (TMAPAPP_Context.BSNK.BIGSyncState == APP_BIG_SYNC_STATE_SYNCHRONIZED)
    {
      ...
      {
        CODEC_ReceiveData(TMAPAPP_Context.BSNK.current_BIS_conn_handles[i], 1, &aLC3OutBuff[0]  + AudioFrameSize/2 + i);
      }
    }
  }
#endif /*((APP_TMAP_ROLE & TMAP_ROLE_BROADCAST_MEDIA_RECEIVER) == TMAP_ROLE_BROADCAST_MEDIA_RECEIVER)*/
...
void APP_NotifyTxAudioHalfCplt(void)
{
  ...
  for (i = 0; i< APP_MAX_NUM_CIS; i++)
  {
    if (TMAPAPP_Context.cis_snk_handle[i] != 0xFFFFu)
    {
      CODEC_ReceiveData(TMAPAPP_Context.cis_snk_handle[i], 1, &aLC3OutBuff[0] + i);
    }
  }
#if ((APP_TMAP_ROLE & TMAP_ROLE_BROADCAST_MEDIA_RECEIVER) == TMAP_ROLE_BROADCAST_MEDIA_RECEIVER)
  if (TMAPAPP_Context.bap_role & BAP_ROLE_BROADCAST_SINK)
  {
    if (TMAPAPP_Context.BSNK.BIGSyncState == APP_BIG_SYNC_STATE_SYNCHRONIZED)
    {
      for (i = 0; i< TMAPAPP_Context.BSNK.current_num_bis; i++)
      {
        CODEC_ReceiveData(TMAPAPP_Context.BSNK.current_BIS_conn_handles[i], 1, &aLC3OutBuff[0] + i);
      }
    }
  }
#endif /*((APP_TMAP_ROLE & TMAP_ROLE_BROADCAST_MEDIA_RECEIVER) == TMAP_ROLE_BROADCAST_MEDIA_RECEIVER)*/
...
void APP_NotifyRxAudioCplt(uint16_t AudioFrameSize)
{
  ...
  for (i = 0; i< APP_MAX_NUM_CIS; i++)
  {
    ...
    {
      CODEC_SendData(TMAPAPP_Context.cis_src_handle[i], 1, &aLC3InBuff[0] + AudioFrameSize/2 + i);
    }
  }
}
...
void APP_NotifyRxAudioHalfCplt(void)
{
  ...
  for (i = 0; i< APP_MAX_NUM_CIS; i++)
  {
    if (TMAPAPP_Context.cis_src_handle[i] != 0xFFFFu)
    {
      CODEC_SendData(TMAPAPP_Context.cis_src_handle[i], 1, &aLC3InBuff[0] + i);
    }
  }
}
...
void TMAPAPP_LinkDisconnected(uint16_t Conn_Handle,uint8_t Reason)
{
  ...
  if (p_conn != 0)
  {
    ...
    /* Restart Broadcast Sink if not already synchronized */
    if (TMAPAPP_Context.BSNK.BIGSyncState != APP_BIG_SYNC_STATE_SYNCHRONIZED)
    {
      TMAPAPP_StartSink();
    }
...
uint8_t TMAPAPP_SyncToPA(uint8_t AdvSID, uint8_t *pAdvAddress, uint8_t AdvAddressType)
{
  ...
  Menu_SetSyncState(TMAPAPP_Context.BSNK.ScanState, TMAPAPP_Context.BSNK.PASyncState, TMAPAPP_Context.BSNK.BIGSyncState);
...
uint8_t TMAPAPP_StartSink(void)
{
  ...
  if (ret == BLE_STATUS_SUCCESS)
  {
    ...
    Menu_SetBroadcastSourceName(SourcesToSync[SourceID]);
  }
  Menu_SetSyncState(TMAPAPP_Context.BSNK.ScanState, TMAPAPP_Context.BSNK.PASyncState, TMAPAPP_Context.BSNK.BIGSyncState);
...
void TMAPAPP_NextSource(void)
{
  SourceID = (SourceID + 1) % NUM_SOURCES;
}
...
static void TMAPAPP_CAPNotification(CAP_Notification_Evt_t *pNotification)
{
  ...
  switch(pNotification->EvtOpcode)
  {
    ...
    case CAP_BROADCAST_AUDIO_UP_EVT:
    {
      ...
      if (TMAPAPP_Context.BSNK.base_group.NumSubgroups == 1)
      {
        Menu_SetLanguage(0);
      }
      else
      {
        uint8_t i = 0;
        while (i+1 < TMAPAPP_Context.BSNK.base_subgroups[SubgroupID].MetadataLength)
        {
          if (TMAPAPP_Context.BSNK.base_subgroups[SubgroupID].pMetadata[i+1] == METADATA_LANGUAGE)
          {
            /* Found language metadata */
            Menu_SetLanguage((char *) &TMAPAPP_Context.BSNK.base_subgroups[SubgroupID].pMetadata[i+2]);
            break;
          }
          i += TMAPAPP_Context.BSNK.base_subgroups[SubgroupID].pMetadata[i] + 1;
        }
        if (i+1 >= TMAPAPP_Context.BSNK.base_subgroups[SubgroupID].MetadataLength)
        {
          Menu_SetLanguage(0);
        }
      }
    ...
    case CAP_BROADCAST_AUDIO_DOWN_EVT:
    {
      ...
      if (TMAPAPP_Context.NumConn > 0)
      {
        Menu_SetBroadcastSourceName("");
      }
      else if (TMAPAPP_Context.NumConn == 0
          && TMAPAPP_Context.BSNK.ScanState == APP_SCAN_STATE_IDLE)
      {
        /* Start Scan */
    ...
    case CAP_BROADCAST_SOURCE_ADV_REPORT_EVT:
    {
         ...
         && TMAPAPP_Context.BSNK.BIGSyncState == APP_BIG_SYNC_STATE_IDLE)
      {
        ...
        if (strcmp(&name[0], SourcesToSync[SourceID]) == 0)
        {
          CAP_Broadcast_AddSourceToBASS(data->AdvSID, data->AdvAddressType, (uint8_t *) &data->pAdvAddress[0],
                                        data->BroadcastID, 0, aAPP_BroadcastCode);
          TMAPAPP_SyncToPA(data->AdvSID, (uint8_t *) &data->pAdvAddress[0], data->AdvAddressType);
          TMAPAPP_Context.BSNK.PASyncState = APP_PA_SYNC_STATE_SYNCHRONIZING;
          Menu_SetSyncState(TMAPAPP_Context.BSNK.ScanState, TMAPAPP_Context.BSNK.PASyncState, TMAPAPP_Context.BSNK.BIGSyncState);
        }
      }
            ...
            for (i = 0; i < TMAPAPP_Context.BSNK.base_group.NumSubgroups && status == BLE_STATUS_SUCCESS; i++)
            {
              ...
              for (j = 0; (j < TMAPAPP_Context.BSNK.base_subgroups[i].NumBISes) && (status == BLE_STATUS_SUCCESS); j++)
              {
                status = CAP_Broadcast_ParseBASEBIS(base_data->pBasePayload,
                                                    base_data->BasePayloadLength,
                                                    &(TMAPAPP_Context.BSNK.base_bis[num_total_bis]),
                                                    &(index));
                base_data->pBasePayload += index;
                base_data->BasePayloadLength -= index;
                LOG_INFO_APP("      BIS INDEX : 0x%02x\n",TMAPAPP_Context.BSNK.base_bis[num_total_bis].BIS_Index);
                LOG_INFO_APP("      Codec specific config length : %d bytes\n",TMAPAPP_Context.BSNK.base_bis[num_total_bis].CodecSpecificConfLength);

                Audio_Chnl_Allocation_t channel_alloc = 0x00000000;
                if (TMAPAPP_Context.BSNK.base_bis[num_total_bis].CodecSpecificConfLength > 0u)
                {
                  for (int k = 0;k<TMAPAPP_Context.BSNK.base_bis[num_total_bis].CodecSpecificConfLength;k++)
                  {
                    if (TMAPAPP_Context.BSNK.base_bis[num_total_bis].pCodecSpecificConf[k] > 0u)
                    {
                      LOG_INFO_APP("        Length: 0x%02x\n",TMAPAPP_Context.BSNK.base_bis[num_total_bis].pCodecSpecificConf[k]);
                      LOG_INFO_APP("          Type: 0x%02x\n",TMAPAPP_Context.BSNK.base_bis[num_total_bis].pCodecSpecificConf[k+1u]);
                      LOG_INFO_APP("          Value: 0x");
                      for (int l = 0 ;l<(TMAPAPP_Context.BSNK.base_bis[num_total_bis].pCodecSpecificConf[k]-1);l++)
                      {
                        LOG_INFO_APP("%02x",TMAPAPP_Context.BSNK.base_bis[num_total_bis].pCodecSpecificConf[k+2u+l]);
                      }
                      LOG_INFO_APP("\n");
                    }
                    k+=TMAPAPP_Context.BSNK.base_bis[num_total_bis].pCodecSpecificConf[k];
                  }
                  channel_alloc = LTV_GetConfiguredAudioChannelAllocation(TMAPAPP_Context.BSNK.base_bis[num_total_bis].pCodecSpecificConf,
                                                                          TMAPAPP_Context.BSNK.base_bis[num_total_bis].CodecSpecificConfLength);
                }
                if (channel_alloc == 0x00000000)
                {
                  /* No channel alloc on BIS level, get channel alloc on subgroup level */
                  channel_alloc = LTV_GetConfiguredAudioChannelAllocation(TMAPAPP_Context.BSNK.base_subgroups[num_total_bis].pCodecSpecificConf,
                                                                          TMAPAPP_Context.BSNK.base_subgroups[num_total_bis].CodecSpecificConfLength);
                }
                if(channel_alloc != 0x00000000)
                {
                ...
                if(((TMAPAPP_Context.BSNK.base_group.NumSubgroups == 1) || (SubgroupID == i))
                   && (TMAPAPP_Context.BSNK.BIGSyncState == APP_BIG_SYNC_STATE_IDLE))
                {
                  if (TMAPAPP_Context.BSNK.Audio_Location != 0x00000000)
                  {
                    ...
                    if(channel_alloc != 0x00000000)
                    {
                      ...
                      {
                        TMAPAPP_Context.BSNK.sync_bis_index[TMAPAPP_Context.BSNK.num_sync_bis] = TMAPAPP_Context.BSNK.base_bis[num_total_bis].BIS_Index;
    ...
    {
      BAP_BA_Broadcast_Source_Add_t *source_param = (BAP_BA_Broadcast_Source_Add_t *) pNotification->pInfo;
      char address_str[13];
      uint8_t i;
      LOG_INFO_APP(">>== CAP_BROADCAST_SDE_ADD_SOURCE_REQ_EVT\n");

      memcpy(TMAPAPP_Context.BSNK.aSDEAdvAddress, source_param->aAdvAddress, 6U);
      snprintf(address_str, 13, "%02X%02X%02X%02X%02X%02X",
               source_param->aAdvAddress[5],
               source_param->aAdvAddress[4],
               source_param->aAdvAddress[3],
               source_param->aAdvAddress[2],
               source_param->aAdvAddress[1],
               source_param->aAdvAddress[0]);

      Menu_SetBroadcastSourceName((const char *) address_str);

      for (i = 0; i < source_param->NumSubgroups; i++)
      {
        if (source_param->pSubgroup[i].BisSync != 0)
        {
          /* Set Subgroup ID for source added by Broadcast Assistant */
          SubgroupID = i;
          break;
        }
      }
    ...
    {
      BAP_BA_Broadcast_Source_Modify_t *source_param = (BAP_BA_Broadcast_Source_Modify_t *) pNotification->pInfo;

      if (source_param->PaSync != 0)
      {
        uint8_t i;
        uint8_t j;

        /* Check if Unicast is streaming */
        for (i = 0; i < CFG_BLE_NUM_LINK; i++)
        {
          for (j = 0; j < APP_NUM_SNK_ASE; j++)
          {
            if (TMAPAPP_Context.aASEs[i].aSnkASE[j].state >= ASE_STATE_ENABLING)
            {
              return;
            }
          }

          for (j = 0; j < APP_NUM_SRC_ASE; j++)
          {
            if (TMAPAPP_Context.aASEs[i].aSrcASE[j].state >= ASE_STATE_ENABLING)
            {
              return;
            }
          }
        ...
        char address_str[13];
        snprintf(address_str, 13, "%02X%02X%02X%02X%02X%02X",
                 TMAPAPP_Context.BSNK.aSDEAdvAddress[5],
                 TMAPAPP_Context.BSNK.aSDEAdvAddress[4],
                 TMAPAPP_Context.BSNK.aSDEAdvAddress[3],
                 TMAPAPP_Context.BSNK.aSDEAdvAddress[2],
                 TMAPAPP_Context.BSNK.aSDEAdvAddress[1],
                 TMAPAPP_Context.BSNK.aSDEAdvAddress[0]);
                                             ...
                                             uint32_t ControllerDelay)
{
  ...
  if(p_ase != 0)
  {
   ...
   if (((p_ase->Type == ASE_SOURCE) && (TMAPAPP_Context.bap_role & BAP_ROLE_UNICAST_SERVER)) || \
    ...
    {
       /* check if there is any Source ASE */
       APP_ACL_Conn_t *p_conn = APP_GetACLConn(ACL_ConnHandle);
       for ( i = 0; i < TMAPAPP_Context.NumSrcASEs;i++)
       {
         if (p_conn->pASEs->aSrcASE[i].state >= ASE_STATE_ENABLING)
         {
           pSrcBuff = (uint8_t *)aSrcBuff;
         }
       }
    ...
    {
      param.Decimation = (pSrcBuff == NULL) ? 2 : 1;
    }

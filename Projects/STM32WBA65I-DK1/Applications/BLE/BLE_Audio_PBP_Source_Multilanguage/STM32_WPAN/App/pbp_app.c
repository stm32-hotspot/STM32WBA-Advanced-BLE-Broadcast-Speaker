 ...
 *  48_4_1 is 13
 */
#define BROADCAST_SOURCE_BAP_CONFIG             (5u)
...
#define BROADCAST_NAME_LENGTH                   11
...
uint8_t aPBPAPP_BroadcastName[BROADCAST_NAME_LENGTH] = "Public_TV_1";

/* Advertising Parameters */
BAP_Extended_Advertising_Params_t extended_adv_params = {
  0x00,          /* Advertising SID */
  128,           /* Advertising Interval min (*0.625) */
  128,           /* Adv Interval max (*0.625) */
  0x07,          /* Channel Map */
...
static void PBPAPP_SetupBASE(void);
...
uint8_t PBPAPP_InitSource(void)
{
  ...
  PBPAPP_Context.max_transport_latency = BAP_BROADCAST_MAX_TRANSPORT_LATENCY;
...
uint8_t PBPAPP_StartSource(void)
{
  ...
  snprintf(language_string, 10, "%s | %s", LANGUAGE_1, LANGUAGE_2);
  UTIL_LCD_DisplayStringAt(0, 5+12+2+2+12, (uint8_t *) language_string, CENTER_MODE);
...
{
  uint8_t i;
  uint8_t a_metadata_sub1[9] = {0x03, METADATA_STREAMING_AUDIO_CONTEXTS,
                                STREAMING_AUDIO_CONTEXT & 0xFF,
                                (STREAMING_AUDIO_CONTEXT >> 8) & 0xFF,
                                0x04, METADATA_LANGUAGE,
                                LANGUAGE_1[0], LANGUAGE_1[1], LANGUAGE_1[2]};
  uint8_t a_metadata_sub2[9] = {0x03, METADATA_STREAMING_AUDIO_CONTEXTS,
                                STREAMING_AUDIO_CONTEXT & 0xFF,
                                (STREAMING_AUDIO_CONTEXT >> 8) & 0xFF,
                                0x04, METADATA_LANGUAGE,
                                LANGUAGE_2[0], LANGUAGE_2[1], LANGUAGE_2[2]};
  ...
  PBPAPP_Context.base_subgroups[0].NumBISes = 1;
  ...
  UTIL_MEM_cpy_8(PBPAPP_Context.subgroup_metadata[0], a_metadata_sub1, 9);
  PBPAPP_Context.base_subgroups[0].pMetadata = &PBPAPP_Context.subgroup_metadata[0][0];
  PBPAPP_Context.base_subgroups[0].MetadataLength = 9;

  PBPAPP_Context.base_subgroups[1].NumBISes = 1;
  PBPAPP_Context.base_subgroups[1].pBIS = &(PBPAPP_Context.base_bis[1]);
  PBPAPP_Context.base_subgroups[1].CodecID = 0x0000000006; /* LC3 */
  PBPAPP_Context.base_subgroups[1].CodecSpecificConfLength = 0x0D;
  PBPAPP_Context.base_subgroups[1].pCodecSpecificConf = &(PBPAPP_Context.codec_specific_config_subgroup[1][0]);
  UTIL_MEM_cpy_8(PBPAPP_Context.subgroup_metadata[1], a_metadata_sub2, 9);
  PBPAPP_Context.base_subgroups[1].pMetadata = &PBPAPP_Context.subgroup_metadata[1][0];
  PBPAPP_Context.base_subgroups[1].MetadataLength = 9;

  PBPAPP_Context.base_group.NumSubgroups = 2;
  PBPAPP_Context.base_group.pSubgroups = &(PBPAPP_Context.base_subgroups[0]);

  PBPAPP_Context.encryption = BAP_BROADCAST_ENCRYPTION;
  PBPAPP_Context.broadcast_code[0] = aPBPAPP_BroadcastCode[0];
  PBPAPP_Context.broadcast_code[1] = aPBPAPP_BroadcastCode[1];
  PBPAPP_Context.broadcast_code[2] = aPBPAPP_BroadcastCode[2];
  PBPAPP_Context.broadcast_code[3] = aPBPAPP_BroadcastCode[3];

  PBPAPP_Context.RTN = 1u;
  uint8_t subgroup1_i = 0;
  uint8_t subgroup2_i = 0;

  PBPAPP_Context.base_group.pSubgroups[0].CodecSpecificConfLength = 0;
  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = 0x02;
  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = CODEC_SAMPLING_FREQ;
  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = aPBPAPP_CodecConf[BROADCAST_SOURCE_BAP_CONFIG%16].Freq;
  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = 0x02;
  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = CODEC_FRAME_DURATION;
  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = aPBPAPP_CodecConf[BROADCAST_SOURCE_BAP_CONFIG%16].FrameDuration;
  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = 0x03;
  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = CODEC_OCTETS_PER_CODEC_FRAME;
  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = (uint8_t) aPBPAPP_CodecConf[BROADCAST_SOURCE_BAP_CONFIG%16].OctetsPerCodecFrame;
  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = (uint8_t) (aPBPAPP_CodecConf[BROADCAST_SOURCE_BAP_CONFIG%16].OctetsPerCodecFrame >> 8);

  PBPAPP_Context.base_group.pSubgroups[0].CodecSpecificConfLength += 10;

  PBPAPP_Context.base_group.pSubgroups[1].CodecSpecificConfLength = 0;
  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = 0x02;
  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = CODEC_SAMPLING_FREQ;
  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = aPBPAPP_CodecConf[BROADCAST_SOURCE_BAP_CONFIG%16].Freq;
  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = 0x02;
  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = CODEC_FRAME_DURATION;
  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = aPBPAPP_CodecConf[BROADCAST_SOURCE_BAP_CONFIG%16].FrameDuration;
  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = 0x03;
  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = CODEC_OCTETS_PER_CODEC_FRAME;
  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = (uint8_t) aPBPAPP_CodecConf[BROADCAST_SOURCE_BAP_CONFIG%16].OctetsPerCodecFrame;
  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = (uint8_t) (aPBPAPP_CodecConf[BROADCAST_SOURCE_BAP_CONFIG%16].OctetsPerCodecFrame >> 8);

  PBPAPP_Context.base_group.pSubgroups[1].CodecSpecificConfLength += 10;

  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = 0x02;
  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = CODEC_FRAMES_BLOCKS_PER_SDU;
  PBPAPP_Context.codec_specific_config_subgroup[0][subgroup1_i++] = 1;

  PBPAPP_Context.base_group.pSubgroups[0].CodecSpecificConfLength += 3;

  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = 0x02;
  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = CODEC_FRAMES_BLOCKS_PER_SDU;
  PBPAPP_Context.codec_specific_config_subgroup[1][subgroup2_i++] = 1;

  PBPAPP_Context.base_group.pSubgroups[1].CodecSpecificConfLength += 3;
  ...
  for (i = 0; i < 2; i++)
  {
    ...
    PBPAPP_Context.RTN = MAX(PBPAPP_Context.RTN, aPBPAPP_BroadcastQoSConf[BROADCAST_SOURCE_BAP_CONFIG].RTN);
    PBPAPP_Context.max_transport_latency = MAX( PBPAPP_Context.max_transport_latency,
                                               aPBPAPP_BroadcastQoSConf[BROADCAST_SOURCE_BAP_CONFIG].MaxTpLatency);
    PBPAPP_Context.base_group.PresentationDelay = MAX(PBPAPP_Context.base_group.PresentationDelay,
                                                      aPBPAPP_BroadcastQoSConf[BROADCAST_SOURCE_BAP_CONFIG].PresentationDelay);

    PBPAPP_Context.base_bis[i].CodecSpecificConfLength = 0;
    ...
    PBPAPP_Context.codec_specific_config_bis[i][bis_i++] = 0; /* Mono Location */
    PBPAPP_Context.codec_specific_config_bis[i][bis_i++] = 0;
    PBPAPP_Context.codec_specific_config_bis[i][bis_i++] = 0;
    PBPAPP_Context.codec_specific_config_bis[i][bis_i++] = 0;

    PBPAPP_Context.base_bis[i].CodecSpecificConfLength += 6;
  }
}

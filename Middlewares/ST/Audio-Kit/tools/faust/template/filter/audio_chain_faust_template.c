...
static int32_t s_$(ALGO_NAME_MINUS_STD)_init(audio_algo_t *const pAlgo)
{
  ...
  const $(ALGO_NAME_MINUS_STD)_static_config_t  *const pStaticConfig  = ($(ALGO_NAME_MINUS_STD)_static_config_t  const *)AudioAlgo_getStaticConfig(pAlgo);
  ...
  if (AudioError_isOk(error))
  {
    ...
    pCtx    = ($(ALGO_NAME_MINUS_STD)Ctx_t *)AudioAlgo_malloc(sizeof($(ALGO_NAME_MINUS_STD)Ctx_t), memPool);
    if (pCtx == NULL)
    {

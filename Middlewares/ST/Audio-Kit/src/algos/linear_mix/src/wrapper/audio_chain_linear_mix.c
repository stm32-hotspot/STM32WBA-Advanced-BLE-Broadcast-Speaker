/**
******************************************************************************
* @file    audio_chain_linear_mix.c
* @author  MCD Application Team
* @brief   wrapper of linear_mix algo to match usage inside audio_chain.c
*******************************************************************************
* @attention
*
...
static int32_t s_linear_mix_deinit(audio_algo_t    *const pAlgo);
static int32_t s_linear_mix_init(audio_algo_t      *const pAlgo);
static int32_t s_linear_mix_configure(audio_algo_t *const pAlgo);
static int32_t s_linear_mix_dataInOut(audio_algo_t *const pAlgo);

/* Global variables ----------------------------------------------------------*/
const audio_algo_common_t AudioChainWrp_linear_mix_common =
{
  .pName                     = "linear-mix",
  ...
  .misc.pAlgoDesc            = AUDIO_ALGO_OPT_STR("Mix entries with linear gains which may be negative (permitting to subtract signals for instance)"),
  .misc.pAlgoHelp            = AUDIO_ALGO_OPT_STR("linear-mix"),
  .misc.flags                = AUDIO_ALGO_FLAGS_MISC_OPTIONAL_INPUTS
};

audio_algo_cbs_t AudioChainWrp_linear_mix_cbs =
{
  .init                       = s_linear_mix_init,
  .deinit                     = s_linear_mix_deinit,
  .configure                  = s_linear_mix_configure,
  .dataInOut                  = s_linear_mix_dataInOut,
  .process                    = NULL,
  .control                    = NULL,
...
static int32_t s_linear_mix_init(audio_algo_t *const pAlgo)
{
  ...
  linearMixCtx_t              *const pContext       = (linearMixCtx_t *)AudioAlgo_malloc(sizeof(linearMixCtx_t) + (nbChunkIn * sizeof(sfcContext_t)), AUDIO_MEM_RAMINT);
  linear_mix_dynamic_config_t *const pDynamicConfig = (linear_mix_dynamic_config_t *)AudioAlgo_getDynamicConfig(pAlgo);
  ...
  if (AudioError_isOk(error))
  {
    error = s_linear_mix_configure(pAlgo);
  }

  if (AudioError_isError(error))
  {
    s_linear_mix_deinit(pAlgo);
  }
...
static int32_t s_linear_mix_deinit(audio_algo_t *const pAlgo)
{
  linearMixCtx_t *const pContext = (linearMixCtx_t *)AudioAlgo_getWrapperContext(pAlgo);
...
static int32_t s_linear_mix_configure(audio_algo_t *const pAlgo)
{
  ...
  for (audio_chunk_list_t *pChunkInList = AudioAlgo_getChunksIn(pAlgo); AudioError_isOk(error) && (pChunkInList != NULL); pChunkInList = pChunkInList->next)
  {
    if (pChunkInList->pChunk != NULL)
    {
      error = sfcUpdateContext(&pContext->pSfcContext[chunkId], mix, pGain[confId]);

/**
******************************************************************************
* @file    audio_chain_wola_inv_factory.c
...
static const audio_descriptor_param_t s_wola_inv_staticParamsDesc[] =
{
  {
  ...
  {
    .pDescription             = AUDIO_ALGO_OPT_STR("with or without synthesis window (synthesis window is necessary in case of reconstruction of combination of different signals, for instance in case of beam-forming antennas), must be consistent with wola_fwd config"),
...
static const audio_descriptor_params_t s_wola_inv_staticParamTemplate =
{
  .pParam                     = (audio_descriptor_param_t *)s_wola_inv_staticParamsDesc,
  .nbParams                   = sizeof(s_wola_inv_staticParamsDesc) / sizeof(s_wola_inv_staticParamsDesc[0]),
  .szBytes                    = sizeof(wola_static_config_t),
};

#endif  // AUDIO_CHAIN_ACSDK_USED || AUDIO_CHAIN_CONF_TUNING_CLI_USED

const audio_algo_factory_t AudioChainWrp_wola_inv_factory =
{
  .pStaticParamTemplate  = AUDIO_ALGO_OPT_TUNING(&s_wola_inv_staticParamTemplate),
  .pDynamicParamTemplate = AUDIO_ALGO_OPT_TUNING(NULL),
  .pControlTemplate      = AUDIO_ALGO_OPT_TUNING(NULL),
  .pCapabilities         = &AudioChainWrp_wola_inv_common,
  .pExecutionCbs         = &AudioChainWrp_wola_inv_cbs
};

/**
******************************************************************************
* @file    audio_chain_faust_flanger.c
* @author  MCD Application Team
* @brief   produce a faust-flanger effect
*******************************************************************************
* @attention
*
...
typedef struct
{
  ...
  mydsp               dsp;
  const faust_flanger_dynamic_config_t *pDynamic;
}  faust_flangerCtx_t;

/* Private defines -----------------------------------------------------------*/
/* Private macros ------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
const audio_algo_common_t  AudioChainWrp_faust_flanger_common =
{
  .pName                     = "faust-flanger",
  ...
  .misc.pAlgoDesc            = AUDIO_ALGO_OPT_STR("Simple faust-flanger effect."),
...
*/
static void s_dsp_update(mydsp *pDsp, const faust_flanger_dynamic_config_t *pParams)
{
  pDsp->fCheckbox0 = pParams->bypass;
  pDsp->fCheckbox1 = pParams->invert_flange_sum;
  pDsp->fHslider1 = pParams->speed;
  pDsp->fHslider0 = pParams->depth;
  pDsp->fHslider3 = pParams->feedback;
  pDsp->fHslider4 = pParams->flange_delay;
  pDsp->fHslider5 = pParams->delay_offset;
  pDsp->fHslider2 = pParams->flanger_output_level;

}


/* Private function prototypes -----------------------------------------------*/
static int32_t s_faust_flanger_deinit(audio_algo_t    *const pAlgo);
static int32_t s_faust_flanger_init(audio_algo_t      *const pAlgo);
static int32_t s_faust_flanger_dataInOut(audio_algo_t *const pAlgo);


/* Global variables ----------------------------------------------------------*/
audio_algo_cbs_t AudioChainWrp_faust_flanger_cbs  =
{
  .init                       = s_faust_flanger_init,
  .deinit                     = s_faust_flanger_deinit,
  .configure                  = NULL,
  .dataInOut                  = s_faust_flanger_dataInOut,
  .process                    = NULL,
  .control                    = NULL,
...
__weak int32_t AudioChainWrp_faust_flanger_deinit_ext(audio_algo_t *const pAlgo)
{
  return 1;
}
...
* @return int32_t error code
*/
__weak int32_t AudioChainWrp_faust_flanger_init_ext(audio_algo_t *const pAlgo)
{
  return 1;
}
...
*/
static void s_load_scratch(const faust_flangerCtx_t *pCtx)
{
...
* @param pCtx the class context instance
*/
static void s_save_scratch(const faust_flangerCtx_t *pCtx)
{
...
* @return int32_t error code
*/
static void  plugin_processing_transform(faust_flangerCtx_t *pCtx)
{
...
static int32_t s_faust_flanger_init(audio_algo_t *const pAlgo)
{
  ...
  const faust_flanger_static_config_t  *const pStaticConfig  = (faust_flanger_static_config_t  const *)AudioAlgo_getStaticConfig(pAlgo);
  const faust_flanger_dynamic_config_t *const pDynamicConfig = (faust_flanger_dynamic_config_t const *)AudioAlgo_getDynamicConfig(pAlgo);
  ...
  if (AudioError_isOk(error))
  {
    ...
    pCtx    = (faust_flangerCtx_t *)AudioAlgo_malloc(sizeof(faust_flangerCtx_t), memPool);
    if (pCtx == NULL)
    {
  ...
  if (AudioError_isOk(error))
  {
    ...
    memset(pCtx, 0, sizeof(faust_flangerCtx_t));
  ...
  if (AudioError_isOk(error))
  {
    AudioChainWrp_faust_flanger_init_ext(pAlgo);
  }

  if (AudioError_isError(error))
  {
    s_faust_flanger_deinit(pAlgo);
  }
...
{
  int32_t                            error = AUDIO_ERR_MGNT_NONE;
  faust_flangerCtx_t *const pCtx  = (faust_flangerCtx_t *)AudioAlgo_getWrapperContext(pAlgo);
  ...
  if (pCtx != NULL)
  {
    ...
    AudioChainWrp_faust_flanger_deinit_ext(pAlgo);

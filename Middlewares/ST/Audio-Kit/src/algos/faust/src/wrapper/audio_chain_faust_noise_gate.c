/**
******************************************************************************
* @file    audio_chain_faust_noise_gate.c
...
#include "faust/faust_noise_gate.c"
/*cstat +MISRAC2012-* */
...
typedef struct
{
  ...
  mydsp               dsp;
  const faust_noise_gate_dynamic_config_t *pDynamic;
}  faust_noise_gateCtx_t;

/* Private defines -----------------------------------------------------------*/
/* Private macros ------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
const audio_algo_common_t  AudioChainWrp_faust_noise_gate_common =
{
  .pName                     = "faust-noise-gate",
  ...
  .misc.pAlgoDesc            = AUDIO_ALGO_OPT_STR("Simple faust-noise-gate effect."),
...
*/
static void s_dsp_update(mydsp *pDsp, const faust_noise_gate_dynamic_config_t *pParams)
{
  pDsp->fCheckbox0 = pParams->bypass;
  pDsp->fHslider0 = pParams->threshold;
  pDsp->fHslider1 = pParams->attack;
  pDsp->fHslider3 = pParams->hold;
...
static int32_t s_faust_noise_gate_deinit(audio_algo_t    *const pAlgo);
static int32_t s_faust_noise_gate_init(audio_algo_t      *const pAlgo);
static int32_t s_faust_noise_gate_dataInOut(audio_algo_t *const pAlgo);


/* Global variables ----------------------------------------------------------*/
audio_algo_cbs_t AudioChainWrp_faust_noise_gate_cbs  =
{
  .init                       = s_faust_noise_gate_init,
  .deinit                     = s_faust_noise_gate_deinit,
  .configure                  = NULL,
  .dataInOut                  = s_faust_noise_gate_dataInOut,
  .process                    = NULL,
  .control                    = NULL,
...
__weak int32_t AudioChainWrp_faust_noise_gate_deinit_ext(audio_algo_t *const pAlgo)
{
  return 1;
}
...
* @return int32_t error code
*/
__weak int32_t AudioChainWrp_faust_noise_gate_init_ext(audio_algo_t *const pAlgo)
{
  return 1;
}
...
*/
static void s_load_scratch(const faust_noise_gateCtx_t *pCtx)
{
...
* @param pCtx the class context instance
*/
static void s_save_scratch(const faust_noise_gateCtx_t *pCtx)
{
...
* @return int32_t error code
*/
static void  plugin_processing_transform(faust_noise_gateCtx_t *pCtx)
{
...
static int32_t s_faust_noise_gate_init(audio_algo_t *const pAlgo)
{
  ...
  const faust_noise_gate_static_config_t  *const pStaticConfig  = (faust_noise_gate_static_config_t  const *)AudioAlgo_getStaticConfig(pAlgo);
  const faust_noise_gate_dynamic_config_t *const pDynamicConfig = (faust_noise_gate_dynamic_config_t const *)AudioAlgo_getDynamicConfig(pAlgo);
  ...
  faust_noise_gateCtx_t                         *pCtx           = NULL;
  ...
  if (AudioError_isOk(error))
  {
    ...
    pCtx    = (faust_noise_gateCtx_t *)AudioAlgo_malloc(sizeof(faust_noise_gateCtx_t), memPool);
    if (pCtx == NULL)
    {
  ...
  if (AudioError_isOk(error))
  {
    ...
    memset(pCtx, 0, sizeof(faust_noise_gateCtx_t));
  ...
  if (AudioError_isOk(error))
  {
    AudioChainWrp_faust_noise_gate_init_ext(pAlgo);
  }

  if (AudioError_isError(error))
  {
    s_faust_noise_gate_deinit(pAlgo);
  }
...
{
  int32_t                            error = AUDIO_ERR_MGNT_NONE;
  faust_noise_gateCtx_t *const pCtx  = (faust_noise_gateCtx_t *)AudioAlgo_getWrapperContext(pAlgo);
  ...
  if (pCtx != NULL)
  {
    ...
    AudioChainWrp_faust_noise_gate_deinit_ext(pAlgo);

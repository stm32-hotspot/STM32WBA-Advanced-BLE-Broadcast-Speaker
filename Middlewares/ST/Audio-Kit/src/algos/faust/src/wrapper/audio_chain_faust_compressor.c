/**
******************************************************************************
* @file    audio_chain_faust_compressor.c
...
#include "faust/faust_compressor.c"
/*cstat +MISRAC2012-* */
...
typedef struct
{
  ...
  mydsp               dsp;
  const faust_compressor_dynamic_config_t *pDynamic;
}  faust_compressorCtx_t;

/* Private defines -----------------------------------------------------------*/
/* Private macros ------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
const audio_algo_common_t  AudioChainWrp_faust_compressor_common =
{
  .pName                     = "faust-compressor",
  ...
  .misc.pAlgoDesc            = AUDIO_ALGO_OPT_STR("Simple faust-compressor effect."),
...
*/
static void s_dsp_update(mydsp *pDsp, const faust_compressor_dynamic_config_t *pParams)
{
  pDsp->fCheckbox0 = pParams->bypass;
  pDsp->fHslider3 = pParams->ratio;
  pDsp->fHslider1 = pParams->threshold;
  pDsp->fHslider0 = pParams->attack;
  pDsp->fHslider2 = pParams->release;
  pDsp->fHslider4 = pParams->makeup_gain;

}


/* Private function prototypes -----------------------------------------------*/
static int32_t s_faust_compressor_deinit(audio_algo_t    *const pAlgo);
static int32_t s_faust_compressor_init(audio_algo_t      *const pAlgo);
static int32_t s_faust_compressor_dataInOut(audio_algo_t *const pAlgo);


/* Global variables ----------------------------------------------------------*/
audio_algo_cbs_t AudioChainWrp_faust_compressor_cbs  =
{
  .init                       = s_faust_compressor_init,
  .deinit                     = s_faust_compressor_deinit,
  .configure                  = NULL,
  .dataInOut                  = s_faust_compressor_dataInOut,
  .process                    = NULL,
  .control                    = NULL,
...
__weak int32_t AudioChainWrp_faust_compressor_deinit_ext(audio_algo_t *const pAlgo)
{
  return 1;
}
...
* @return int32_t error code
*/
__weak int32_t AudioChainWrp_faust_compressor_init_ext(audio_algo_t *const pAlgo)
{
  return 1;
}
...
*/
static void s_load_scratch(const faust_compressorCtx_t *pCtx)
{
...
* @param pCtx the class context instance
*/
static void s_save_scratch(const faust_compressorCtx_t *pCtx)
{
...
* @return int32_t error code
*/
static void  plugin_processing_transform(faust_compressorCtx_t *pCtx)
{
...
static int32_t s_faust_compressor_init(audio_algo_t *const pAlgo)
{
  ...
  const faust_compressor_static_config_t  *const pStaticConfig  = (faust_compressor_static_config_t  const *)AudioAlgo_getStaticConfig(pAlgo);
  const faust_compressor_dynamic_config_t *const pDynamicConfig = (faust_compressor_dynamic_config_t const *)AudioAlgo_getDynamicConfig(pAlgo);
  ...
  faust_compressorCtx_t                         *pCtx           = NULL;
  ...
  if (AudioError_isOk(error))
  {
    ...
    pCtx    = (faust_compressorCtx_t *)AudioAlgo_malloc(sizeof(faust_compressorCtx_t), memPool);
    if (pCtx == NULL)
    {
  ...
  if (AudioError_isOk(error))
  {
    ...
    memset(pCtx, 0, sizeof(faust_compressorCtx_t));
  ...
  if (AudioError_isOk(error))
  {
    AudioChainWrp_faust_compressor_init_ext(pAlgo);
  }

  if (AudioError_isError(error))
  {
    s_faust_compressor_deinit(pAlgo);
  }
...
{
  int32_t                            error = AUDIO_ERR_MGNT_NONE;
  faust_compressorCtx_t *const pCtx  = (faust_compressorCtx_t *)AudioAlgo_getWrapperContext(pAlgo);
  ...
  if (pCtx != NULL)
  {
    ...
    AudioChainWrp_faust_compressor_deinit_ext(pAlgo);

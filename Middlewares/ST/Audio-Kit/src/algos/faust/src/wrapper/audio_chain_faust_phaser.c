/**
******************************************************************************
* @file    audio_chain_faust_phaser.c
...
typedef struct
{
  ...
  mydsp               dsp;
  const faust_phaser_dynamic_config_t *pDynamic;
}  faust_phaserCtx_t;

/* Private defines -----------------------------------------------------------*/
/* Private macros ------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
const audio_algo_common_t  AudioChainWrp_faust_phaser_common =
{
  .pName                     = "faust-phaser",
  ...
  .misc.pAlgoDesc            = AUDIO_ALGO_OPT_STR("Simple faust-phaser effect."),
...
*/
static void s_dsp_update(mydsp *pDsp, const faust_phaser_dynamic_config_t *pParams)
{
  pDsp->fCheckbox0 = pParams->bypass;
  pDsp->fCheckbox1 = pParams->invert_internal_phaser_sum;
  pDsp->fCheckbox2 = pParams->vibrato_mode;
  pDsp->fHslider1 = pParams->speed;
  pDsp->fHslider0 = pParams->notch_depth__intensity_;
  pDsp->fHslider6 = pParams->feedback_gain;
  pDsp->fHslider5 = pParams->notch_width;
  pDsp->fHslider2 = pParams->min_notch1_freq;
  pDsp->fHslider3 = pParams->max_notch1_freq;
  pDsp->fHslider4 = pParams->notch_freq_ratio;
  pDsp->fHslider7 = pParams->phaser_output_level;

}


/* Private function prototypes -----------------------------------------------*/
static int32_t s_faust_phaser_deinit(audio_algo_t    *const pAlgo);
static int32_t s_faust_phaser_init(audio_algo_t      *const pAlgo);
static int32_t s_faust_phaser_dataInOut(audio_algo_t *const pAlgo);


/* Global variables ----------------------------------------------------------*/
audio_algo_cbs_t AudioChainWrp_faust_phaser_cbs  =
{
  .init                       = s_faust_phaser_init,
  .deinit                     = s_faust_phaser_deinit,
  .configure                  = NULL,
  .dataInOut                  = s_faust_phaser_dataInOut,
  .process                    = NULL,
  .control                    = NULL,
...
__weak int32_t AudioChainWrp_faust_phaser_deinit_ext(audio_algo_t *const pAlgo)
{
  return 1;
}
...
* @return int32_t error code
*/
__weak int32_t AudioChainWrp_faust_phaser_init_ext(audio_algo_t *const pAlgo)
{
  return 1;
}
...
*/
static void s_load_scratch(const faust_phaserCtx_t *pCtx)
{
...
* @param pCtx the class context instance
*/
static void s_save_scratch(const faust_phaserCtx_t *pCtx)
{
...
* @return int32_t error code
*/
static void  plugin_processing_transform(faust_phaserCtx_t *pCtx)
{
...
static int32_t s_faust_phaser_init(audio_algo_t *const pAlgo)
{
  ...
  const faust_phaser_static_config_t  *const pStaticConfig  = (faust_phaser_static_config_t  const *)AudioAlgo_getStaticConfig(pAlgo);
  const faust_phaser_dynamic_config_t *const pDynamicConfig = (faust_phaser_dynamic_config_t const *)AudioAlgo_getDynamicConfig(pAlgo);
  ...
  if (AudioError_isOk(error))
  {
    ...
    pCtx    = (faust_phaserCtx_t *)AudioAlgo_malloc(sizeof(faust_phaserCtx_t), memPool);
    if (pCtx == NULL)
    {
  ...
  if (AudioError_isOk(error))
  {
    ...
    memset(pCtx, 0, sizeof(faust_phaserCtx_t));
  ...
  if (AudioError_isOk(error))
  {
    AudioChainWrp_faust_phaser_init_ext(pAlgo);
  }

  if (AudioError_isError(error))
  {
    s_faust_phaser_deinit(pAlgo);
  }
...
{
  int32_t                            error = AUDIO_ERR_MGNT_NONE;
  faust_phaserCtx_t *const pCtx  = (faust_phaserCtx_t *)AudioAlgo_getWrapperContext(pAlgo);
  ...
  if (pCtx != NULL)
  {
    ...
    AudioChainWrp_faust_phaser_deinit_ext(pAlgo);

/**
******************************************************************************
* @file    audio_chain_faust_reverb_dattorro.c
...
#include "faust/faust_reverb_dattorro.c"
/*cstat +MISRAC2012-* */
...
typedef struct
{
  ...
  mydsp               dsp;
  const faust_reverb_dattorro_dynamic_config_t *pDynamic;
}  faust_reverb_dattorroCtx_t;

/* Private defines -----------------------------------------------------------*/
/* Private macros ------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
const audio_algo_common_t  AudioChainWrp_faust_reverb_dattorro_common =
{
  .pName                     = "faust-reverb-dattorro",
  ...
  .misc.pAlgoDesc            = AUDIO_ALGO_OPT_STR("Simple faust-reverb-dattorro effect."),
...
*/
static void s_dsp_update(mydsp *pDsp, const faust_reverb_dattorro_dynamic_config_t *pParams)
{
  pDsp->fVslider6 = pParams->prefilter;
  pDsp->fVslider5 = pParams->diffusion_1;
  pDsp->fVslider4 = pParams->diffusion_wall2;
  pDsp->fVslider2 = pParams->diffusion_wall3;
  pDsp->fVslider0 = pParams->diffusion_wall4;
  pDsp->fVslider1 = pParams->decay_rate;
  pDsp->fVslider3 = pParams->damping;
  pDsp->fVslider7 = pParams->dry_wet_mix;
  pDsp->fVslider8 = pParams->level;

}


/* Private function prototypes -----------------------------------------------*/
static int32_t s_faust_reverb_dattorro_deinit(audio_algo_t    *const pAlgo);
static int32_t s_faust_reverb_dattorro_init(audio_algo_t      *const pAlgo);
static int32_t s_faust_reverb_dattorro_dataInOut(audio_algo_t *const pAlgo);


/* Global variables ----------------------------------------------------------*/
audio_algo_cbs_t AudioChainWrp_faust_reverb_dattorro_cbs  =
{
  .init                       = s_faust_reverb_dattorro_init,
  .deinit                     = s_faust_reverb_dattorro_deinit,
  .configure                  = NULL,
  .dataInOut                  = s_faust_reverb_dattorro_dataInOut,
  .process                    = NULL,
  .control                    = NULL,
...
__weak int32_t AudioChainWrp_faust_reverb_dattorro_deinit_ext(audio_algo_t *const pAlgo)
{
  return 1;
}
...
* @return int32_t error code
*/
__weak int32_t AudioChainWrp_faust_reverb_dattorro_init_ext(audio_algo_t *const pAlgo)
{
  return 1;
}
...
*/
static void s_load_scratch(const faust_reverb_dattorroCtx_t *pCtx)
{
...
* @param pCtx the class context instance
*/
static void s_save_scratch(const faust_reverb_dattorroCtx_t *pCtx)
{
...
* @return int32_t error code
*/
static void  plugin_processing_transform(faust_reverb_dattorroCtx_t *pCtx)
{
...
static int32_t s_faust_reverb_dattorro_init(audio_algo_t *const pAlgo)
{
  ...
  const faust_reverb_dattorro_static_config_t  *const pStaticConfig  = (faust_reverb_dattorro_static_config_t  const *)AudioAlgo_getStaticConfig(pAlgo);
  const faust_reverb_dattorro_dynamic_config_t *const pDynamicConfig = (faust_reverb_dattorro_dynamic_config_t const *)AudioAlgo_getDynamicConfig(pAlgo);
  ...
  faust_reverb_dattorroCtx_t                         *pCtx           = NULL;
  ...
  if (AudioError_isOk(error))
  {
    ...
    pCtx    = (faust_reverb_dattorroCtx_t *)AudioAlgo_malloc(sizeof(faust_reverb_dattorroCtx_t), memPool);
    if (pCtx == NULL)
    {
  ...
  if (AudioError_isOk(error))
  {
    ...
    memset(pCtx, 0, sizeof(faust_reverb_dattorroCtx_t));
  ...
  if (AudioError_isOk(error))
  {
    AudioChainWrp_faust_reverb_dattorro_init_ext(pAlgo);
  }

  if (AudioError_isError(error))
  {
    s_faust_reverb_dattorro_deinit(pAlgo);
  }
...
{
  int32_t                            error = AUDIO_ERR_MGNT_NONE;
  faust_reverb_dattorroCtx_t *const pCtx  = (faust_reverb_dattorroCtx_t *)AudioAlgo_getWrapperContext(pAlgo);
  ...
  if (pCtx != NULL)
  {
    ...
    AudioChainWrp_faust_reverb_dattorro_deinit_ext(pAlgo);

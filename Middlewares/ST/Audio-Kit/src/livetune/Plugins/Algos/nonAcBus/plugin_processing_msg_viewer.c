/**
******************************************************************************
* @file          plugin_processing_msg_viewer.c
...
/*
{
   "Description":"Show messages coming from acMsg pins",
   "Name":"Message-Viewer",
   "Icon":"icon-elem-Tool.png",
   "httpHelp":"message-viewer",
   "PinDefIn":[
      {
         "Type":"acMsg",
         "Name":"Msg",
         "ConnectionMax":256,
         "Description": "Accept several connections"
       }
   ]
}
*/
static char_t                tElementDescription[] = "{\"httpHelp\":\"message-viewer\",\"Description\":\"Show messages coming from acMsg pins\",\"Name\":\"Message-Viewer\",\"Icon\":\"icon-elem-Tool.png\",\"PinDefIn\":[{\"Type\":\"acMsg\",\"Name\":\"Msg\",\"ConnectionMax\":256,\"Description\":\"Accept several connections\"}],\"Params\":[{\"Description\":\"Message Window\",\"Type\":0,\"Name\":\"Message\",\"Control\":\"message\"}]}";
...
static livetune_builder_result plugin_processing_builder(livetune_db_instance *pInstance, uint32_t cmd)
{
  switch (cmd)
  {
    case (uint32_t)LIVETUNE_PIPE_START:
    {
...
static void plugin_test_control_processing_constructor(livetune_db_instance *pInstance)
{
  ...
  {
    memset(pInstance->pUserData, 0, sizeof(st_plugin_element_viewer));
  }
  else
  {
 ...
 */
static uint32_t plugin_msg_viewer_event_cb(ST_Handle hInst, ST_Message_Event evt, st_message_param wParam, st_message_param lParam)
{
  ...
  if (evt == ST_EVT_REGISTER_ELEMENTS)
  {
    switch (livetune_db_element_json_register(&gpInstance->hDesignerDB, tElementDescription, NULL, interface_test_control, &pElement))
    {
      ...
      case ST_NOT_IMPL:
        ...
        break;
      default:
        ST_TRACE_ERROR("cannot register algo %s", "Message-Viewer");
        break;
    }
  }
...
ST_PLUGIN_DECLARE(plugin_msg_viewer_event_cb);

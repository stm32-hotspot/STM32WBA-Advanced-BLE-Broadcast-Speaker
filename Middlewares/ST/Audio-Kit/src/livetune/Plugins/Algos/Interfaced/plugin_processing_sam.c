...
/* Private defines -----------------------------------------------------------*/
#define ALGO_PLUGIN_CB plugin_sam_event_cb
...
/* weak function to allow user to add other behavior in liveTune mode --------*/
int32_t plugin_processing_control_sam_user_cb(acAlgo hAlgo);
...
/* Private functions ------------------------------------------------------- */
__weak int32_t plugin_processing_control_sam_user_cb(acAlgo hAlgo)
{
  return 0;
}
...
static int32_t plugin_processing_control_sam(acAlgo hAlgo)
{
  if (hAlgo)
  {
    plugin_processing_control_sam_user_cb(hAlgo);
    ...
    {
      ST_VERIFY(acAlgoGetControl(hAlgo, "beamSelect", &pClsExt->beamSelect) == 0);
      ST_VERIFY(acAlgoGetControl(hAlgo, "psdMicDb", &pClsExt->psdMicDb) == 0);
      ST_VERIFY(acAlgoGetControl(hAlgo, "mixerGainLin", &pClsExt->mixerGainLin) == 0);
      ...
      if (pScratchBody)
      {
        ...
        snprintf(tlocalScratch, sizeof(tlocalScratch), "Energy of mic signal in Db = %6.2f (for chosen bandwidth using idBandStart) Selected beam index %d", (double)pClsExt->psdMicDb, pClsExt->beamSelect);
        strcat(pScratchBody, tlocalScratch);

        /* build mandatory json fields to identify the MsgViewer instance to send to */
        livetune_helper_format_clear(tlocalScratch);
        snprintf(tlocalScratch, sizeof(tlocalScratch), "\"From\":\"%s\",\"To\":\"%s\"", pCls->hBuilder.pInstance->pInstanceName, pClsExt->pConnectedMsg->pInstance->pInstanceName);
  ...
  switch (cmd)
  {
    case LIVETUNE_PIPE_INIT_GRAPH_ELEMENT:
    {
      ...
      if (pInstance->hAc.bValid)
      {
        ...
        livetune_ac_wrapper_acAlgoSetCommonConfig(&pCls->hBuilder, "controlCb", (void *)plugin_processing_control_sam); /*cstat !MISRAC2012-Rule-11.1 cast a callback in pointer is mandatory for this API */

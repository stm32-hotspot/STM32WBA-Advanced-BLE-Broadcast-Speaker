...
/* Private defines -----------------------------------------------------------*/
#define ALGO_PLUGIN_CB plugin_rms_event_cb

/*
{
"PinDefOut":[{"Type":"acMsg","Name":"Msg","Description":"Must be connected to an acMsg viewer"}]
}
*/
...
static int32_t plugin_processing_control_rms(acAlgo hAlgo)
{
  if (hAlgo)
  {
    ...
    {
      ST_VERIFY(acAlgoGetControl(hAlgo, "rms", &pClsExt->pRmsArray) == 0);
      ST_VERIFY(acAlgoGetControl(hAlgo, "nbChannels", &pClsExt->nbChannels) == 0);
      /* alloc the memory for the the msg body */

      char    tlocalScratch[LOCAL_SCRATCH_MSG];
      char    tlocalVar[LOCAL_SCRATCH_MSG] = "";

      uint32_t szScratchBody = LOCAL_SCRATCH_MSG * pClsExt->nbChannels;
      char_t  *pScratchBody  = st_os_mem_alloc(ST_Mem_Type_Designer, szScratchBody);
      if (pScratchBody)
      {
        memset(pScratchBody, 0, szScratchBody);
        for (uint32_t i = 0; i < pClsExt->nbChannels; i++)
        {
          livetune_helper_format_clear(tlocalScratch);
          float rms = (pClsExt->pRmsArray[i] > 0.0f) ? (20.0f * log10f(pClsExt->pRmsArray[i])) : -100.0f; /*cstat !MISRAC2012-Dir-4.11_a !MISRAC2012-Rule-22.8 false positif, the range is checked */
          snprintf(tlocalScratch, sizeof(tlocalScratch), "channel %d: %2.2f%% = %2.1f dB\n", i, (double)pClsExt->pRmsArray[i], (double)rms);
          strcat(pScratchBody, tlocalScratch);
          /* Enable a variable watcher */
          if (i) { strcat(tlocalVar, " "); } /* add a space to split variable */
          char *pVars = strchr(tlocalVar, 0);
          snprintf(pVars, sizeof(tlocalScratch), "ch%d:%2.1fdB", i, (double)rms); /* the syntax is Key:Value*/
        }
  ...
  switch (cmd)
  {
    case LIVETUNE_PIPE_INIT_GRAPH_ELEMENT:
    {
      ...
      if (pInstance->hAc.bValid)
      {
        ...
        livetune_ac_wrapper_acAlgoSetCommonConfig(&pCls->hBuilder, "controlCb", (void *)plugin_processing_control_rms); /*cstat !MISRAC2012-Rule-11.1 cast a callback in pointer is mandatory for this API */
